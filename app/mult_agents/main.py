...
def build_agent(config: AppConfig, prompt_key: str, temperature: float, tools: list):
...
def build_agents(config: AppConfig) -> AgentBundle:
    ...
    return AgentBundle(
        intent_router=build_agent(config, "intent_router", 0.0, []),
        planner=build_agent(config, "plan", 0.3, []),
        scout_web=build_agent(config, "web_search", 0.4, []),
        scout_local=build_agent(config, "local_rag", 0.4, []),
        evidence_judge=build_agent(config, "deep_dive", 0.2, []),
        analyst=build_agent(config, "analyze", 0.3, []),
        direct_responder=build_agent(config, "direct_answer", 0.2, []),
        writer=build_agent(config, "write", 0.4, []),
    )
