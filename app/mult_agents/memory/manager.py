...
from ..openai_clients import build_chat_openai
from ..rag.core import OpenAICompatibleEmbeddings
...
class MemoryManager:
    def __init__(
        ...
        Args:
            short_term_ttl: 短期记忆的过期时间，单位秒。Redis 后端会用它设置 key 过期时间。
            short_term_max_messages: 单个会话保留的短期消息上限，超过后触发压缩。
            short_term_summary_threshold: 压缩后继续保留的最近消息数，较早消息会进入滚动摘要。
            db_path: SQLite 降级存储路径。PostgreSQL 不可用或未启用时，长期记忆会使用它。
            tenant_id: 默认租户 ID，用于多租户场景下隔离记忆数据。
            short_term_backend: 短期记忆后端，可选 postgres、redis 或内存降级。
            long_term_backend: 长期记忆后端，可选 postgres、sqlite 或 disabled。
            long_term_scope: 长期记忆作用域。user 表示跨 thread 共享，thread 表示只在当前会话内使用。
            save_conversation_task: 是否把每轮对话作为 Episodic task 写入长期记忆。
            enable_milvus: 是否为长期记忆启用 Milvus 向量索引和语义召回。
            redis_url: Redis 连接串，仅 short_term_backend 或 checkpointer 使用 Redis 时需要。
            postgres_dsn: PostgreSQL 连接串，用于短期记忆、长期记忆和 checkpointer。
            milvus_host: Milvus 主机地址。为空时不会初始化 Milvus。
            milvus_port: Milvus gRPC 服务端口。
            milvus_collection: Milvus collection 名称，用于存储长期记忆向量。
            embedding_api_key: Embedding 和摘要模型使用的 API Key。
            base_url: OpenAI 兼容接口地址。
            embedding_model: 写入和检索 Milvus 时使用的 embedding 模型名称。
            summary_model: 短期记忆滚动压缩时使用的摘要模型名称。
        """
        ...
        if self.enable_milvus and self.enable_long_term:
            self._init_milvus(milvus_host, milvus_port, milvus_collection, embedding_api_key, base_url, embedding_model)
    ...
    def _init_summary_llm(self, api_key: Optional[str], base_url: str, summary_model: str) -> None:
        if not api_key:
            return
        try:
            self._summary_llm = build_chat_openai(
                model=summary_model,
                temperature=0.1,
                api_key=api_key,
                base_url=base_url,
