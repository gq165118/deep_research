...
class AppConfig:
    ...
    def from_file(path: str | Path | None = None) -> "AppConfig":
        ...
        api_key = AppConfig._resolve_str(data, "api_key", "AGICTO_API_KEY", "")
        if not api_key:
            raise ValueError(
                f"缺少 AGICTO_API_KEY 配置，请在 {config_path} 中填写 api_key，或设置环境变量 AGICTO_API_KEY"
            )
        base_url = AppConfig._resolve_str(data, "base_url", "AGICTO_BASE_URL", "https://api.agicto.cn/v1")
        model = AppConfig._resolve_str(data, "model", "AGICTO_MODEL", "gpt-5.4")
    ...
    def from_env() -> "AppConfig":
        ...
        api_key = AppConfig._resolve_str(data, "api_key", "AGICTO_API_KEY", "")
        if not api_key:
            raise ValueError("缺少 AGICTO_API_KEY 环境变量")
        base_url = AppConfig._resolve_str(data, "base_url", "AGICTO_BASE_URL", "https://api.agicto.cn/v1")
        model = AppConfig._resolve_str(data, "model", "AGICTO_MODEL", "gpt-5.4")
        thread_id = AppConfig._resolve_str(data, "thread_id", "THREAD_ID", "default")
        user_id = AppConfig._resolve_str(data, "user_id", "USER_ID", "default_user")
